...
impl<'a> Lexer<'a> {
    ...
    fn next_octal_digit(&mut self) -> LexerResult<u32> {
        self.next_char_expect(|c| ('0'..='9').contains(&c), LexerError::ExpectOctDigit)
            .map(|c| c as u32 - '0' as u32)
    }

    // decimalDigit = "0" … "9"
    fn next_decimal_digit(&mut self) -> LexerResult<u32> {
        self.next_char_expect(|c| ('0'..='9').contains(&c), LexerError::ExpectDecDigit)
            .map(|c| c as u32 - '0' as u32)
    }
    ...
    fn next_token_inner(&mut self) -> LexerResult<Token> {
        ...
        if let Some(c) = self.next_char_if(|c| c.is_ascii_punctuation()) {
            ...
            if c == ':' && self.next_char_if_eq(':') {
                return Ok(Token::DoubleColon);
            }
            if c == '-' && self.next_char_if_eq('>') {
                return Ok(Token::FnReturns);
            }
